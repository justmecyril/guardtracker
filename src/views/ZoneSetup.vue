<template>
  <div class="max-w-screen-sm px-4 py-10 mx-auto">
    ...
    <form
      ...
      class="flex flex-col p-8 mx-auto rounded-md shadow-lg bg-light-grey text-light-green"
    >
      <h1 class="mb-4 text-3xl">Zone</h1>
      <div class="flex flex-col mb-2">
        <label for="zoneName" class="text-lg">Zone Name</label>
        <input
          type="text"
          id="zoneName"
    ...
    <div class="my-8">
      ...
      >
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-xl font-bold leading-none">Zone list</h3>
        </div>
        <router-link
          :to="{ name: 'ZoneDetails', params: { zoneId: zone.id } }"
          v-for="zone in zones"
          :key="zone.id"
        >
          <ul>
...
export default {
  setup() {
    ...
    const createZone = async () => {
      try {
        await store.dispatch("addZone", {
          zone: zoneName.value,
          description: zoneDescription.value,
        });
        statusMsg.value = "Success: Zone Created!";
        zoneName.value = "";
